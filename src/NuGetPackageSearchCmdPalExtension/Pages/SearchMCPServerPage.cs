...
{
    internal sealed partial class SearchMCPServerPage : DynamicListPage, IDisposable
    {
        ...
        public override ICommandItem? EmptyContent
        {
            get
            {
                if (_isError)
                {
                    ...
                    {
                        Title = "Error loading dotnet MCP Servers",
                        Icon = new IconInfo("\uea39"),
                        Subtitle = "An error occurred while fetching dotnet MCP Servers.",
                ...
                return new CommandItem
                {
                    Title = "Search for a dotnet MCP Servers",
                    Icon = new IconInfo("\uea39"),
                    Subtitle = "Search for a dotnet MCP Servers",
                    Command = new NoOpCommand()
                }; ;
            }
        }
        ...
        private static async Task<IReadOnlyList<ListItem>> ProcessSearchAsync(string searchText, CancellationToken cancellationToken)
        {
            ...
            var url = $"https://azuresearch-usnc.nuget.org/query?take=20&packageType=mcpserver&q={Uri.EscapeDataString(searchText)}";
            ...
            foreach (var package in dataElement.EnumerateArray())
            {
                ...
                results.Add(new ListItem
                {
                    ...
                    [
                        ...
                        new CommandContextItem(new CopyTextCommand($$"""
                                                                     {
                                                                         servers:'{
                                                                            {{id}}:'{
                                                                                 type:"stdio",
